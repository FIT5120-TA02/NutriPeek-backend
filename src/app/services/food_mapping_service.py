...
class FoodMappingService:
    ...
    async def map_food_items(
        ...
        Returns:
            Tuple containing:
                - Dictionary mapping food names to their nutrient data
        ...
        try:
            ...
            async with get_async_session() as db:
                # Map food names to nutrient data using the improved CRUD function
                mapped_items, unmapped_items = await food_nutrient_crud.map_food_items(
                    db, food_names=food_names
                )

                # Store in inventory if requested
                if store_in_inventory and mapped_items and children_profile_id:
                    await self._store_in_inventory(
                        db, mapped_items, children_profile_id
                    )
